#!/bin/bash
#
# Setup script for 腾讯智能文档 MCP Skill (内部 OpenClaw 版本) 一体化配置与授权脚本
#
# 功能：
...
#
# 说明：
#   - 本 skill 的名称是 tencent-smartcanvas
#   - 但底层 MCP 仍然是腾讯文档统一服务，mcporter 中的注册名保持为 tencent-docs
#
# 用法（供 AI Agent 调用）：
#   第一步：检查状态（立即返回，不阻塞）
...
#
# 注意：
#   某些 Agent / 沙箱执行环境会在首条命令结束后清理后台子进程。
#   在此类环境中，第一阶段启动的后台轮询可能无法存活到第二阶段。
#   本脚本会在第二阶段检测轮询进程是否仍然存活；若已丢失，会自动切换为前台轮询补偿。
#
...
# ── 判断后台轮询进程是否仍存活 ────────────────────────────────────────────────
_tdoc_bg_poll_alive() {
    if [[ ! -f "$_TDOC_PID_FILE" ]]; then
        return 1
    fi

    local pid
    pid=$(cat "$_TDOC_PID_FILE" 2>/dev/null)
    if [[ -z "$pid" ]]; then
        return 1
    fi
...
_tdoc_save_token() {
    ...
    # 使用传入的 token 写入 mcporter 配置（tencent-smartcanvas）
    ...
    if mcporter list 2>&1 | grep -q "$_TDOC_SERVICE_NAME"; then
        echo "✅ tencent-smartcanvas 配置验证成功！"
        echo ""
        mcporter list | grep -A 1 "$_TDOC_SERVICE_NAME" || true
    else
        echo "⚠️  tencent-smartcanvas 配置验证失败，请检查网络或 Token 是否有效"
    fi
    ...
    echo "   mcporter call ${_TDOC_SERVICE_NAME} manage.search_file"
    echo ""
...
_tdoc_wait_token() {
    ...
    # 某些受控执行环境会在上一条命令结束时清理后台子进程。
    # 如果第二阶段发现轮询进程已经丢失，但授权码仍在，则切换为前台轮询补偿。
    if [[ ! -f "$_TDOC_TOKEN_FILE" && ! -f "$_TDOC_ERR_FILE" && -f "$_TDOC_CODE_FILE" ]] && ! _tdoc_bg_poll_alive; then
        _tdoc_poll_for_token >/dev/null 2>&1 &
        local recovery_pid=$!
        echo "$recovery_pid" > "$_TDOC_PID_FILE"
    fi
