...
function buildMemoryAnswer(
  ...
  const asksAboutPreference = /偏好|风格|tone|voice|preference|写作/.test(
    normalizedQuery,
  );
  const asksAboutMount = /挂载|集成|agent|mount|integration|调用|接入/.test(
    normalizedQuery,
  );
  const asksAboutRisk = /风险|follow|待跟进|todo|next|下一步|candidate/.test(
    normalizedQuery,
  );
...
export default function V2MemoryHomePage() {
  ...
  const recentSignals = useMemo(
    () =>
      timelineEntries
        .filter((entry) => entry.event.stage !== "durable")
        .slice(0, 3),
    [timelineEntries],
  );
  ...
  const copy =
    lang === "zh"
      ...
      : {
          ...
          askAiTitle: "Ask AI",
          assistantPlaceholder:
            "Ask anything about your memory, for example “What am I pushing forward lately?”",
          send: "Send",
          quickPrompts: [
            ...
            },
            {
              ...
              previewItems: [
                ...
                },
                {
                  label: "2",
                  title:
                    "Refine cross-platform interaction quality and multi-device adaptation",
                  meta: "Deadline: May · Priority: Medium",
                },
              ],
            },
              ...
              <div className="mt-4 space-y-2.5">
                {section.previewType === "memory" &&
                  section.previewItems.map((item) => (
                    ...
                    >
                      ...
                      <p className="mt-1.5 text-xs leading-6 text-stone-700">
                        {item.body}
                      </p>
                    </div>
                  ))}
                ...
                {section.previewType === "knowledge" &&
                  section.previewItems.map((item) => (
                    ...
                    >
                      <div className="min-w-0 text-[13px] font-medium leading-6 text-stone-800">
                        {item.title}
                      </div>
                      {item.badge && (
                  ...
                  onClick={() => {
                    void navigate(
                      `/v2/memory/timeline?event=${signal.event.id}${
                        signal.signal.assetId
                          ? `&asset=${signal.signal.assetId}`
                          : ""
                      }&from=memory-home`,
                    );
                  }}
