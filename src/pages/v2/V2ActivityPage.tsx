            ...
            {sortedActivities.length === 0 ? (
              ...
              sortedActivities.map((activity) => (
                <button
                  key={activity.id}
                  onClick={() =>
                    activity.docId && navigate(`/v2/doc/${activity.docId}`)
                  }
                  className="w-full text-left rounded-[1.35rem] border border-stone-200 bg-stone-50/60 px-4 py-4 transition-all hover:border-stone-300 hover:bg-stone-50"
                >
          ...
          <div className="mt-5 space-y-3">
            {formingSignals.map((signal) => (
              <button
                ...
                onClick={() =>
                  navigate(
                    `/v2/memory/timeline?doc=${signal.docId}&from=activity`,
                  )
                }
