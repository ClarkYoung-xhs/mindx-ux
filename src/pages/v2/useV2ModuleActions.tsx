import { FileText, MessageSquare, RefreshCw, Upload, Waypoints } from 'lucide-react';
...
const normalizeDocType = (type: string) => type.toLowerCase().replace(/\s+/g, '');
...
export function useWorkspaceCreateActions(): V2HeaderActionItem[] {
  ...
  const createDoc = (type: 'Smart Doc' | 'Markdown' | 'Table' | 'Whiteboard' | 'Form') => {
    const id = `d${Date.now()}`;
    ...
    navigate(`/document?id=${id}&type=${normalizeDocType(type)}&from=v2-workspace`);
  };

  return useMemo(
    () => [
      { id: 'smart-doc', label: lang === 'zh' ? '智能文档' : 'Smart Doc', icon: getDocTypeIcon('Smart Doc', 16), onSelect: () => createDoc('Smart Doc') },
      { id: 'table', label: lang === 'zh' ? '表格' : 'Table', icon: getDocTypeIcon('Table', 16), onSelect: () => createDoc('Table') },
      { id: 'whiteboard', label: lang === 'zh' ? '白板' : 'Whiteboard', icon: getDocTypeIcon('Whiteboard', 16), onSelect: () => createDoc('Whiteboard') },
      { id: 'form', label: lang === 'zh' ? '表单' : 'Form', icon: getDocTypeIcon('Form', 16), onSelect: () => createDoc('Form') },
      { id: 'markdown', label: 'Markdown', icon: getDocTypeIcon('Markdown', 16), onSelect: () => createDoc('Markdown') },
    ],
    [lang]
  );
}
...
export function useMemoryCreateActions(): V2HeaderActionItem[] {
  ...
  const createMemoryIntake = (kind: 'markdown' | 'chat' | 'sync') => {
    ...
    const localized = {
      markdown: {
        docName: lang === 'zh' ? '新接入的 Markdown 资料' : 'Imported markdown note',
        docType: 'Markdown',
        category: 'manual-upload' as const,
        sourceKind: 'document' as const,
        title: lang === 'zh' ? '从 Markdown 中捕获到新的记忆线索' : 'Captured a new memory cue from markdown',
        summary:
          lang === 'zh'
            ? '刚导入了一份 Markdown 资料，系统已经从中提取出值得继续沉淀的线索。'
            : 'A markdown note was imported and the system pulled out a memory cue worth refining.',
        assetTitle:
          lang === 'zh' ? '从导入文档里提取出的新判断' : 'New judgment extracted from the imported markdown',
        assetSummary:
          lang === 'zh'
        ...
        evidence:
          lang === 'zh'
            ...
            : ['Markdown document imported.', 'The first extraction pass has completed.'],
        nextStep:
          lang === 'zh'
            ? '继续补充上下文，并判断它是否应该升格为稳定资产。'
            : 'Add context and decide whether this should graduate into a durable asset.',
        tags: ['markdown', 'memory'],
        entities: lang === 'zh' ? ['Markdown 资料', '记忆接入'] : ['Markdown note', 'memory intake'],
        participants: [currentUser.name],
      },
      chat: {
        docName: lang === 'zh' ? '新接入的聊天记录' : 'Imported chat transcript',
        docType: 'Chat Log',
        category: 'third-party-sync' as const,
        sourceKind: 'chat' as const,
        title: lang === 'zh' ? '从聊天记录中抽出新的记忆候选' : 'Pulled a new memory candidate out of chat history',
        summary:
          lang === 'zh'
        ...
        assetTitle:
          lang === 'zh' ? '聊天记录里的用户偏好候选' : 'Preference candidate distilled from chat history',
        assetSummary:
          lang === 'zh'
        ...
        evidence:
          lang === 'zh'
            ? ['聊天记录已接入。', '偏好线索已被标记出来。']
            : ['Chat transcript imported.', 'A preference signal has been marked.'],
        nextStep:
          lang === 'zh'
        ...
        entities: lang === 'zh' ? ['聊天记录', '偏好'] : ['chat transcript', 'preference'],
        participants: [currentUser.name, 'Claude Assistant'],
      },
      sync: {
        docName: lang === 'zh' ? '新建的外部同步源' : 'New External Sync Source',
        docType: 'Smart Doc',
        category: 'cloud-collaboration' as const,
        sourceKind: 'report' as const,
        title: lang === 'zh' ? '从外部同步源接入了新的记忆流' : 'A new memory stream arrived from an external sync source',
        summary:
          lang === 'zh'
        ...
        assetTitle:
          lang === 'zh' ? '外部同步带来的新记忆入口' : 'A new memory entry point created by external sync',
        assetSummary:
          lang === 'zh'
        ...
        entities: lang === 'zh' ? ['外部同步', '记忆写入'] : ['external sync', 'memory writeback'],
    ...
    const newDataSource: MemoryDataSource = {
      ...
      status: kind === 'sync' ? 'syncing' : kind === 'chat' ? 'reviewing' : 'ready',
    ...
    const newAsset: MemoryAsset = {
      id: assetId,
      type: kind === 'chat' ? 'preference' : kind === 'sync' ? 'constraint' : 'insight',
...
export function useIntegrationCreateActions(): V2HeaderActionItem[] {
  const { lang } = useLanguage();
  const { agentConnections, memoryAssets, setAgentConnections, setMountedMemoryPacks, setAgentWritebacks } =
    useMindXDemo();
  ...
  const createMountPack = () => {
    ...
    const nextPack: MountedMemoryPack = {
      ...
      assetIds,
      mountedByConnectionIds: agentConnections.slice(0, 2).map(connection => connection.id),
    };
