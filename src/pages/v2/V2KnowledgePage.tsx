...
const localizedSourceCopy: Record<
  string,
  Record<"zh" | "en", LocalizedSourceCopy>
> = {
  src1: {
    zh: {
    ...
    en: {
      ...
      quote:
        "If we promote Knowledge to top-level navigation now, users will read it as a second product instead of Memory growing up.",
    },
  },
  src2: {
    zh: {
    ...
    en: {
      ...
      quote:
        "People do not trust memory because it is bigger. They trust it when they can see where a card came from and why it still holds.",
    },
  },
  src3: {
    zh: {
    ...
    en: {
      ...
      quote:
        "Knowledge is not a parallel module. It is where recurring judgments inside Memory become reusable assets.",
    },
  },
  src4: {
    zh: {
    ...
    en: {
      ...
      quote:
        "Lead with the judgment. The product should sound like it already knows what matters, not like it is reading a brochure.",
    },
  },
  src5: {
    zh: {
    ...
    en: {
      ...
      quote:
        "If open loops can live everywhere, the system will feel thoughtful but not governable.",
    },
  },
  src6: {
    zh: {
    ...
    en: {
      ...
      quote:
        "Today stopped being about adding more cards and became about deciding what deserves to become durable.",
    },
  },
};

const localizedAssetCopy: Record<
  string,
  Record<"zh" | "en", LocalizedCardCopy>
> = {
  mem1: {
    zh: {
    ...
    en: {
      title:
        "Knowledge should remain inside Memory instead of becoming top-level navigation",
      summary:
        "For the current mindX 2.0 IA, Knowledge is a structured shelf inside Memory rather than a standalone top-level entry.",
  ...
  mem2: {
    ...
    en: {
      title:
        "Workspace should remain the outer wrapper while Memory explains the user underneath",
      summary:
        "The current entry strategy keeps Workspace as the operating shell, while Memory surfaces who the user is and what should persist over time.",
  ...
  mem3: {
    ...
    en: {
      ...
      summary:
        "The stable writing pattern is to state the conclusion and tradeoff first, then explain the UI mechanics or system details underneath it.",
  ...
  mem4: {
    ...
    en: {
      title:
        "The overall voice should stay direct, restrained, and non-corporate",
      summary:
        "Both brand and product language should avoid inflated AI phrasing, internal-report tone, and empty positioning statements.",
  ...
  mem5: {
    ...
    en: {
      title:
        "User trust comes from provenance, not from “better memory” claims",
      summary:
        "Long-term memory becomes credible when users can see the source, review trail, and current validity of each card.",
  ...
  mem6: {
    ...
    en: {
      title:
        "Memory must grow out of workflow instead of floating outside Workspace",
      summary:
        "The system only earns repeated use when memory emerges from real documents, collaboration, and decisions rather than becoming a separate archive.",
  ...
  mem7: {
    ...
    en: {
      ...
      summary:
        "All product framing should reinforce continuity instead of reinvention, so users read 2.0 as an upgrade rather than a disconnected system.",
  ...
  mem8: {
    ...
    en: {
      ...
      summary:
        "This three-part frame is sufficient for the current stage of mindX 2.0 Memory; future features should fit inside it instead of creating another primary section.",
  ...
  mem9: {
    ...
    en: {
      ...
      summary:
        "The layer names are now clear, but the product still lacks explicit criteria for when project judgments become principles and when principles become anchors.",
  ...
  mem10: {
    ...
    en: {
      title:
        "It is still unresolved whether follow-ups belong in Knowledge or Timeline",
      summary:
        "Current follow-ups look like assets but behave like workflow. If this boundary stays fuzzy, the Knowledge page will keep getting noisier.",
...
function localizedSource(source: SourceLink, lang: "zh" | "en") {
  return (
    localizedSourceCopy[source.id]?.[lang] ?? {
...
function localizedAsset(asset: MemoryAsset, lang: "zh" | "en") {
  ...
  return {
    ...
    summary: shouldPreferLiveContent
      ? asset.summary
      : (base?.summary ?? asset.summary),
    freshness: shouldPreferLiveContent
      ? asset.freshness
      : (base?.freshness ?? asset.freshness),
    evidence: shouldPreferLiveContent
      ? asset.evidence
      : (base?.evidence ?? asset.evidence),
  };
}
...
export default function V2KnowledgePage() {
  ...
  const [assetCategoryFilter, setAssetCategoryFilter] = useState<
    MemoryAsset["libraryCategory"] | null
  >(null);
  ...
  const selectedCard = selectedCardId
    ? (assetCardMap[selectedCardId] ?? null)
    : null;
      ...
      memoryAssets
        .filter((asset) => {
          ...
          const matchesCategory =
            !assetCategoryFilter ||
            asset.libraryCategory === assetCategoryFilter;
          const matchesSource =
            !sourceFilterId ||
            asset.sourceIds.some(
              (sourceId) =>
                sourceMap[sourceId]?.dataSourceId === sourceFilterId,
            );
  ...
  const allResonanceCards = useMemo(() => {
    ...
    return definitions
      ...
      .filter((item): item is { card: V2CardRecord; tone: V2CardTone } =>
        Boolean(item),
      );
  }, [assetCardMap, lang]);
  ...
  const assetCategoryCards = useMemo(() => {
    ...
    return definitions.map((definition) => ({
      ...definition,
      count: memoryAssets.filter(
        (asset) => asset.libraryCategory === definition.id,
      ).length,
    }));
  }, [lang, memoryAssets]);
  ...
  const activeCategory = useMemo(
    () =>
      assetCategoryCards.find(
        (category) => category.id === assetCategoryFilter,
      ) ?? null,
  ...
  const openSource = (source: V2SourceRef) => {
    if (source.storage === "memory" && source.dataSourceId) {
      navigate(
        `/document?source=data_source&dataSourceId=${source.dataSourceId}&from=knowledge`,
      );
      return;
    }
  ...
  const runMemoAction = (
    cardId: string,
    action: "refine" | "tags" | "related",
  ) => {
    const asset = assetMap[cardId];
    if (!asset) {
      return lang === "zh"
        ? "这张记忆暂时还没有足够上下文。"
        : "This memory card does not have enough context yet.";
    }
    ...
    if (action === "refine") {
      ...
      setMemoryAssets((prev) =>
        prev.map((item) =>
          item.id === cardId ? { ...item, summary: refinedSummary } : item,
        ),
      );
  ...
  const askMemoAgent = (cardId: string, question: string) => {
    ...
    if (
      lowerQuestion.includes("下一步") ||
      lowerQuestion.includes("next") ||
      lowerQuestion.includes("怎么做")
    ) {
      return lang === "zh"
    ...
    if (
      lowerQuestion.includes("标签") ||
      lowerQuestion.includes("tag") ||
      lowerQuestion.includes("关联")
    ) {
      return relatedTitles.length > 0
        ? lang === "zh"
  ...
  return (
    <>
          ...
          <div className="border-b border-stone-200">
            <div className="flex items-center gap-6 overflow-x-auto">
              {[
                {
                  key: "assets" as const,
                  label: lang === "zh" ? "资产库" : "Assets",
                },
                {
                  key: "tags" as const,
                  label: lang === "zh" ? "标签分类" : "Tags",
                },
                {
                  key: "insights" as const,
                  label: lang === "zh" ? "AI 察觉" : "AI Noticing",
                },
              ].map((tab) => (
                <button
                  key={tab.key}
                  ...
                  {resonanceCards[0] && (
                    <div className="md:col-span-2 lg:col-span-3 lg:row-span-2">
                      <V2ResonanceCard
                        ...
                        featured
                        onClick={() =>
                          setSelectedCardId(resonanceCards[0].card.id)
                        }
                      />
                    </div>
                  )}
                  {resonanceCards[1] && (
                    <div className="md:col-span-2 lg:col-span-3">
                      <V2ResonanceCard
                        ...
                        lang={lang}
                        onClick={() =>
                          setSelectedCardId(resonanceCards[1].card.id)
                        }
                      />
                    </div>
                  )}
                  {resonanceCards[2] && (
                    <div className="md:col-span-2 lg:col-span-3">
                      <V2ResonanceCard
                        ...
                        lang={lang}
                        onClick={() =>
                          setSelectedCardId(resonanceCards[2].card.id)
                        }
                      />
                    </div>
                  )}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
                {assetCategoryCards.map((asset) => (
                  <motion.button
                    ...
                    onClick={() =>
                      setAssetCategoryFilter(
                        assetCategoryFilter === asset.id ? null : asset.id,
                      )
                    }
                  ...
                  >
                    ...
                    <div className="text-[15px] font-semibold leading-tight text-zinc-900">
                      {asset.label}
                    </div>
              ...
              <div className="mt-10">
                <div className="mb-6 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-xs font-bold text-zinc-400">
                    ...
                    <span>
                      {lang === "zh"
                        ? `${filteredAssetCards.length} 条记录`
                        : `${filteredAssetCards.length} records`}
                    </span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
              <div className="space-y-8 lg:col-span-3">
                ...
                <div className="rounded-[2rem] border border-zinc-200 bg-[#F9F9F8] p-6">
                  <h4 className="mb-2 text-xs font-bold text-zinc-700">
                    结构化建议
                  </h4>
                  <p className="text-[11px] leading-relaxed text-zinc-600">
                    {getTagSuggestion(
                      tagFilter ?? allTags[0] ?? "memory",
                      lang,
                    )}
                  </p>
                </div>
      ...
      <V2CardDetailModal
        ...
        editDocumentHref={
          selectedCard ? getCardDocumentHref(selectedCard.id) : undefined
        }
        ...
        showInsightActions={Boolean(
          selectedCardId &&
          knowledgeMode === "insights" &&
          insightCardIds.has(selectedCardId),
        )}
