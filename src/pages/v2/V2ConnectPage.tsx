...
export default function V2ConnectPage() {
  ...
  const { agentConnections, agentWritebacks, memoryAssets, mountedMemoryPacks } = useMindXDemo();
  ...
  const stats = useMemo(
    ...
    [agentConnections.length, agentWritebacks.length, copy.stats.connected, copy.stats.mounted, copy.stats.writebacks, mountedMemoryPacks.length]
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {stats.map(stat => (
          <div key={stat.label} className="rounded-2xl border border-stone-200 bg-stone-50/70 px-4 py-4">
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-400">
              {stat.label}
            </div>
            <div className="mt-2 text-2xl font-semibold tracking-tight text-stone-900">{stat.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_380px]">
        <section className="rounded-[1.75rem] border border-stone-200 bg-white p-6 shadow-[0_10px_30px_rgba(28,25,23,0.04)]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold tracking-tight text-stone-900">{copy.connections}</h2>
          </div>
          <div className="mt-5 space-y-3">
            {agentConnections.map(connection => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-stone-900">{connection.name}</span>
                      <span className="rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold text-stone-500">
                        {connection.provider}
                      </span>
                    </div>
                    <p className="mt-2 text-sm leading-6 text-stone-600">{connection.description}</p>
                ...
                <button
                  onClick={() => navigate(`/v2/workspace?doc=${connection.latestWorkspaceDocId}&from=agent`)}
        ...
        <div className="space-y-6">
          <section className="rounded-[1.75rem] border border-stone-200 bg-[#F7F7F5] p-6 shadow-[0_10px_30px_rgba(28,25,23,0.03)]">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-stone-900">{copy.mounts}</h2>
            </div>
            <div className="mt-5 space-y-3">
              {mountedMemoryPacks.map(pack => {
                const mountedAgents = pack.mountedByConnectionIds
                  .map(connectionId => agentConnections.find(connection => connection.id === connectionId))
                  .filter((connection): connection is (typeof agentConnections)[number] => Boolean(connection));

                return (
                  <div key={pack.id} className="rounded-[1.35rem] border border-stone-200 bg-white px-4 py-4">
                    <div className="text-sm font-semibold text-stone-900">{pack.name}</div>
                    <p className="mt-2 text-sm leading-6 text-stone-600">{pack.description}</p>
          ...
          <section className="rounded-[1.75rem] border border-stone-200 bg-white p-6 shadow-[0_10px_30px_rgba(28,25,23,0.04)]">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-stone-900">{copy.writebacks}</h2>
            </div>
            <div className="mt-5 space-y-3">
              {agentWritebacks.map(writeback => {
                const relatedAsset = memoryAssets.find(asset => asset.id === writeback.assetId);
                return (
                  <div key={writeback.id} className="rounded-[1.35rem] border border-stone-200 bg-stone-50/70 px-4 py-4">
                    <div className="text-sm font-semibold text-stone-900">{writeback.title}</div>
                    <p className="mt-2 text-sm leading-6 text-stone-600">{writeback.detail}</p>
                    {relatedAsset && (
                      <button
                        onClick={() => navigate(`/v2/memory/knowledge?asset=${relatedAsset.id}`)}
                        className="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-stone-700"
                      >
                        {copy.openKnowledge}
                        <ArrowRight className="h-3.5 w-3.5" />
