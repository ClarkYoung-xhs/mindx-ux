...
import { ChevronLeft, ChevronRight, Clock3, FileText, MessageSquare, Sparkles } from 'lucide-react';
...
function stageBadgeClasses(stage: 'captured' | 'candidate' | 'durable') {
  if (stage === 'durable') return 'border-emerald-200 bg-emerald-50 text-emerald-700';
...
function timelinePeople(sources: SourceLink[]) {
  ...
  return sources.reduce<Array<{ name: string; kind: 'human' | 'agent' }>>((people, source) => {
...
export default function V2MemoryPage() {
  ...
  const timelineGroups = useMemo(
    () =>
      visibleTimelineEntries.reduce<Array<{ label: string; items: typeof visibleTimelineEntries }>>((groups, entry) => {
  ...
  const metrics = useMemo(() => {
    const collaborators = new Set(
      visibleTimelineEntries.flatMap(entry => entry.people.map(person => person.name))
    );
  ...
  const openSource = (source: V2SourceRef) => {
    if (source.storage === 'memory' && source.dataSourceId) {
      navigate(`/document?source=data_source&dataSourceId=${source.dataSourceId}&from=memory`);
      return;
    }
  ...
  return (
    <>
          ...
          <div className="rounded-[1.75rem] border border-stone-200 bg-white p-6 shadow-[0_10px_30px_rgba(28,25,23,0.04)]">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <div className="rounded-full border border-stone-200 bg-stone-50 px-3 py-2 text-xs font-medium text-stone-600">
                {lang === 'zh' ? `摘要节点 ${metrics.storyBeats}` : `${metrics.storyBeats} story beats`}
              </div>
              <div className="rounded-full border border-stone-200 bg-stone-50 px-3 py-2 text-xs font-medium text-stone-600">
                {lang === 'zh' ? `协作角色 ${metrics.collaborators}` : `${metrics.collaborators} collaborators`}
              </div>
              <div className="rounded-full border border-stone-200 bg-stone-50 px-3 py-2 text-xs font-medium text-stone-600">
                {lang === 'zh' ? `已沉淀 ${metrics.durableMemories}` : `${metrics.durableMemories} durable`}
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-lg font-semibold tracking-tight text-stone-900">
                  {lang === 'zh' ? '今天最值得记住的主线' : 'The main thread worth keeping today'}
                </div>
              </div>
            </div>
            <p className="mt-4 text-sm leading-7 text-stone-600">{reportText}</p>
          </aside>
        </section>
        ...
        {focusedDoc && (
          <section className="rounded-[1.75rem] border border-stone-200 bg-[#F7F7F5] p-5 shadow-[0_10px_30px_rgba(28,25,23,0.03)]">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-stone-400">
                  {lang === 'zh' ? '来自 Workspace 的过滤' : 'Filtered from Workspace'}
                </div>
                <div className="mt-2 text-lg font-semibold text-stone-900">{focusedDoc.name}</div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => navigate(`/v2/workspace?doc=${focusedDoc.id}&from=memory`)}
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            ...
                            {entry.people.length > 0 && (
                              <div className="flex items-center gap-2 text-sm text-stone-500">
                                <V2AvatarStack people={entry.people.slice(0, 3)} size={28} overlap={-8} />
                          ...
                          <p className="mt-3 text-sm leading-7 text-stone-700">{entry.signal.summary}</p>
                          ...
                          <div className="mt-5 flex flex-wrap items-center gap-3">
                            <button
                              ...
                              onClick={() =>
                                openSource({
                                  id: entry.primarySource?.id ?? entry.signal.eventId,
      ...
      <V2CardDetailModal
        ...
        onOpenRelated={cardId => setSelectedCard(assetCardMap[cardId] ?? null)}
