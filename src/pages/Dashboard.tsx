...
export default function Dashboard() {
    ...
    try {
      ...
      for (const item of itemsToExtract) {
        ...
        const baseInstruction = lang === "zh"
          ? `\n\n【核心约束】：\n1. 从给定的 Text 中**直接捡取**能改变我们预测该主体未来行为的原文片段。不要改写、不要总结、不要用第三人称复述。\n2. 将每条信号归入以下四类之一："preference"（偏好）、"judgment"（判断）、"choice"（选择）、"decision"（决策）。\n3. 你的回答必须是合法的 JSON 格式！包含唯一的根级属性 "insights" (值为数组)。\n   数组的每一项必须包含如下三个字段：\n   - "type": 只能为 "preference" | "judgment" | "choice" | "decision"\n   - "text": (从原文中一字不差地摘出的完整句子或段落，禁止改写)\n   - "context": (短句标注：该信号发生在何种情境/讨论/业务语境下)\n4. 宁可少提取，不可编造或改写。如果某段话不能原样摘出一个完整表意的片段，就跳过它。\n5. 除了只输出合法的 JSON 对象外，严禁夹带任何文本。`
          : `\n\n[CORE CONSTRAINT]:\n1. **Pick** exact passages from the given Text that would change our prediction of the subject's future behavior. Do NOT rephrase, summarize, or use third person.\n2. Classify each into one of: "preference", "judgment", "choice", "decision".\n3. Return ONLY valid JSON with a root "insights" array. Each item MUST have exactly THREE keys:\n   - "type": "preference" | "judgment" | "choice" | "decision"\n   - "text": (The verbatim, unmodified quote from the original text — complete sentences only)\n   - "context": (Brief annotation: under what situation/discussion was this said?)\n4. Prefer fewer, high-fidelity picks over many lossy summaries. Skip anything that cannot be quoted verbatim.\n5. Output ONLY the JSON object, nothing else.`;
  ...
  const handleOpenProfileEditor = (key: "whoami" | "goal") => {
    navigate(`/document?type=text&backTab=profile&source=${key}_doc`);
  };
...
Command: Download the zip package from https://cdn.addon.tencentsuite.com/static/tencent-docs.zip and unzip it, help me install this skill, and then set the environment variable TENCENT_DOCS_TOKEN="8678a272066b4156ae2ad95c2709cc2b".`;
  ...
  return (
    <div className="h-screen bg-white text-stone-800 flex font-sans selection:bg-stone-200 overflow-hidden">
      ...
      {/* Modals for CustomNode and Profile are removed since they navigate directly to DocumentEditor */}
